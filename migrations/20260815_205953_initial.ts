...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   CREATE TYPE "public"."enum_products_frame_shape" AS ENUM('round', 'square', 'rectangle', 'aviator', 'cat-eye', 'oval', 'hexagonal', 'wayfarer');
  CREATE TYPE "public"."enum_products_gender" AS ENUM('men', 'women', 'unisex', 'kids');
  CREATE TYPE "public"."enum_products_frame_color" AS ENUM('black', 'tortoise', 'gold', 'silver', 'brown', 'blue', 'transparent', 'rose-gold');
  CREATE TYPE "public"."enum_products_pricing_mode" AS ENUM('simple', 'options');
  ...
  CREATE TABLE "categories" (
  	...
  	"headline_ar" varchar NOT NULL,
  	"headline_en" varchar,
  	"outlined_pill" varchar NOT NULL,
  	"outlined_pill_ar" varchar NOT NULL,
  	"outlined_pill_en" varchar,
  ...
  CREATE TABLE "products" (
  	...
  	"frame_shape" "enum_products_frame_shape",
  	"gender" "enum_products_gender",
  	"frame_color" "enum_products_frame_color",
  	"badge" varchar,
  	"badge_ar" varchar,
  	"badge_en" varchar,
  	"rating" numeric DEFAULT 0 NOT NULL,
  	"reviews" numeric DEFAULT 0 NOT NULL,
  	"description" varchar,
  	"description_ar" varchar,
  	"description_en" varchar,
  	"primary_image_id" integer,
  	"artboard_image_id" integer,
  	"featured_rank" numeric,
  	"default_stock_type" varchar,
  	"default_weight_grams" numeric,
  	"default_declared_value" numeric,
  	"default_fragile" boolean,
  	"default_insurance_enabled" boolean,
  	"pricing_mode" "enum_products_pricing_mode" DEFAULT 'simple' NOT NULL,
  	"simple_price" numeric,
  	"simple_original_price" numeric,
  ...
  CREATE TABLE "wilayas" (
  	...
  	"office_delivery_price" numeric NOT NULL,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  );
  ...
  ALTER TABLE "products" ADD CONSTRAINT "products_artboard_image_id_media_id_fk" FOREIGN KEY ("artboard_image_id") REFERENCES "public"."media"("id") ON DELETE set null ON UPDATE no action;
  ...
  CREATE INDEX "products_artboard_image_idx" ON "products" USING btree ("artboard_image_id");
  ...
  CREATE UNIQUE INDEX "wilayas_name_idx" ON "wilayas" USING btree ("name");
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  DROP TYPE "public"."enum_products_frame_shape";
  DROP TYPE "public"."enum_products_gender";
  DROP TYPE "public"."enum_products_frame_color";
  DROP TYPE "public"."enum_products_pricing_mode";
